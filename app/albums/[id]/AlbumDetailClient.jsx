...
import { Check, Copy, File, FolderOpen, Loader2, RotateCcw, WandSparkles } from 'lucide-react';
...
function detectTagWriteSupport() {
  ...
  if (!isChromiumFamily) return { supported: false, reason: 'タグ書き込みは Chrome / Edge 系ブラウザでご利用ください。' };
...
function formatEditionDisplay(editionValue, packageSpecValue) {
  const edition = editionValue === null || editionValue === '' ? '-' : editionValue;
  const packageSpec = packageSpecValue === null || packageSpecValue === '' ? '' : packageSpecValue;
...
export default function AlbumDetailClient() {
  const { toggleTheme: onToggleTheme } = useTheme();
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    router.replace(getAlbumRoutePath(album, id));
  }, [album, id, router]);
  ...
  const handleEditionChange = useCallback(
    (event) => {
      ...
      router.push(`/albums/${nextId}`);
    },
    [canonicalAlbumId, router]
  );
        ...
        <span className="min-w-0 break-words text-left">
          ...
          {list.map((person, idx) => (
            <span key={`${token}-${person.id ?? 'text'}-${idx}`}>
              {person.id ? (
                <Link
                  href={getArtistTracksRoutePath(person, role)}
                  className={linkClass}
                >
                  {person.name}
                </Link>
  ...
  const renderAlbumArtistCard = () => (
    <div className={`${panelMutedClass} border-slate-200/50 bg-white/40 dark:bg-black/20 backdrop-blur-md md:col-span-2`}>
      <p className={detailLabelClass}>アルバムアーティスト</p>
      <div className={detailValueWrapClass}>
        {shouldLinkAlbumArtist ? (
          <Link
            href={getArtistAlbumsRoutePath(album?.album_artist)}
  ...
  const handleWriteTags = async () => {
    if (isWriting) return;
    if (!workerReady) return setTagError('タグ書き込み engine を準備中です。しばらく待って再実行してください。');
      ...
      <div className={`${pageCardClass} max-w-7xl`}>
          ...
          actions={
            <div className="flex items-center gap-3 w-full md:w-auto">
              <button
                ...
                title={showCopyButtons ? 'コピーボタンを非表示にする' : 'コピーボタンを表示する'}
              >
                <Copy className={`h-3.5 w-3.5 ${showCopyButtons ? 'animate-pulse' : ''}`} />
                {showCopyButtons ? 'コピーボタン：表示' : 'コピーボタン：非表示'}
              </button>
              <button
                type="button"
                onClick={() => router.back()}
                className={primaryButtonClass + " flex-1 md:flex-none !px-4 md:!px-8"}
              >
                {'一覧へ戻る'}
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-[auto_1fr] items-start">
          ...
          <div className={`${panelClass} p-3 bg-white/40 dark:bg-black/20 border-slate-200/50 shadow-none rounded-[32px] w-fit backdrop-blur-md`}>
              ...
              {currentCover?.url ? (
                <>
                  <img
                    ...
                    className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-110 group-hover:blur-sm"
                  />
                  <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-black/60 p-6 opacity-0 transition-all duration-500 group-hover:opacity-100 backdrop-blur-xl">
                    <img
                      src={currentCover.url}
                      alt={album.title ?? 'album cover zoom'}
                      className="max-h-full max-w-full object-contain rounded-xl shadow-2xl scale-90 group-hover:scale-100 transition-transform duration-700"
                    />
                  </div>
                </>
              ) : (
            ...
            {hasMultipleCovers && (
              <div className="mt-4 flex flex-wrap gap-3 max-w-[288px]">
                {coverOptions.map((cover, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative aspect-square w-14 overflow-hidden rounded-2xl border-2 transition-all duration-300 ${selected ? 'border-sky-500 shadow-[0_0_15px_rgba(14,165,233,0.5)] scale-110' : 'border-transparent opacity-40 hover:opacity-100'}`}
                      title={label}
                    >
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
              ...
              {renderDetailTextCard('発売日', formatDateDisplay(album?.release_date), 'album-release-date')}
              ...
              {shouldShowSeries && (
                <div className={`${panelMutedClass} border-slate-200/50 bg-white/40 dark:bg-black/20 backdrop-blur-md`}>
                  ...
                  <div className={detailValueWrapClass}>
                    <Link
                      href={`/series/${album.series.public_id ?? album.series.id}/albums`}
                      className={linkedPeopleClass + " " + detailTextClass}
                    >
              ...
              {shouldShowContent && (
                <div className={`${panelMutedClass} border-slate-200/50 bg-white/40 dark:bg-black/20 backdrop-blur-md`}>
                  ...
                  <div className={detailValueWrapClass}>
                    <p className={detailTextClass}>{showValue(contentName)}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {albumCommentText !== '' && (
              <div className={`${panelMutedClass} border-slate-200/50 bg-white/40 dark:bg-black/20 backdrop-blur-md`}>
                <details className="group" open={commentOpen} onToggle={(event) => setCommentOpen(event.currentTarget.open)}>
                  ...
                  <div className="mt-4 border-t border-slate-200/30 dark:border-white/10 pt-4 text-sm font-bold leading-relaxed text-slate-600 dark:text-white/80">
                ...
                {albumLinks.map((link) => (
                  <li key={link.id} className="inline-flex items-center gap-3 rounded-2xl border border-slate-200/50 bg-white/40 px-3 py-2 sm:px-4 sm:py-2.5 dark:border-white/5 dark:bg-black/20 backdrop-blur-xl transition-all hover:border-sky-500 group">
                    ...
                    {link.url ? (
                      ...
                      <span className="text-sm font-bold text-slate-500 dark:text-white/40">{albumLinkTitle(link)}</span>
                    )}
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-sm font-bold text-slate-400 dark:text-white/20 uppercase tracking-widest">No Links Found.</p>
            )}
          </InfoCard>
        )}
          ...
          {support.supported && shouldShowTagOptionDetails && (
            <div className="mt-6 sm:mt-8 space-y-5 sm:space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 items-start">
                <div className="space-y-4 md:col-span-1">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    ...
                    />
                    <span className="text-sm font-bold text-slate-700 dark:text-white/60 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">ジャケット画像を埋め込む（2MBまで）</span>
                  </label>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer group pt-2">
                    <input
                      ...
                      disabled={isWriting}
                    />
                    <span className="text-sm font-bold text-slate-700 dark:text-white/60 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">タグ書き込み時にファイル名をリネームする</span>
                  </label>

                  {shouldShowDiscFolderNameOption && (
                    <label className="flex items-center gap-3 cursor-pointer group pl-8 pt-1">
                      <input
                        type="checkbox"
                        className="w-4 h-4 rounded border-slate-300 dark:border-white/10 bg-white/20 text-sky-500 focus:ring-sky-500/20"
                        checked={includeDiscNumberInFolderName}
                        onChange={(event) => setIncludeDiscNumberInFolderName(event.target.checked)}
                        disabled={isWriting}
                      />
                      <span className="text-xs font-bold text-slate-500 dark:text-white/40 group-hover:text-slate-700 dark:group-hover:text-white/60 transition-colors">フォルダ名にディスク番号を含める</span>
                    </label>
                  )}
                  ...
                  {shouldShowEditionInAlbumNameOption && (
                    <label className="flex items-center gap-3 cursor-pointer group pt-2">
                      <input
                        ...
                        onChange={(event) => setIncludeEditionInAlbumName(event.target.checked)}
                        disabled={isWriting}
                      />
                      <span className="text-sm font-bold text-slate-700 dark:text-white/60 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">アルバム名に形態を含める</span>
                    </label>
                  )}
                </div>
              ...
              <div className="space-y-2">
                {renameOnWrite && (
                  <p className="text-sm font-bold text-amber-600 dark:text-amber-400 leading-relaxed uppercase">
          ...
          <section className="mt-6 sm:mt-8 space-y-4 sm:space-y-6">
            ...
            {trackListGroups.length > 0 ? (
              ...
              <div className="py-20 text-center">
                <p className="text-sm font-bold text-slate-400 dark:text-white/20 uppercase tracking-widest">トラック情報がありません。</p>
              </div>
            )}
          </section>
        )}
