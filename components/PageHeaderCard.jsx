    ...
    <>
      {shouldShowTopControls ? (
        <div className={['mx-auto mb-3 flex items-center justify-between gap-2 lg:justify-start', maxWidthClass].join(' ')}>
          {backLabel ? <button type="button" onClick={onBack} className={backButtonClass}>{backLabel}</button> : <span />}
        </div>
      ) : null}
      ...
      <section className={[heroPanelClass, sectionClassName].filter(Boolean).join(' ')}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1 space-y-3">
            ...
            {title || subtitle ? <div className="space-y-2">{title ? <h1 className={titleClassName}>{title}</h1> : null}{subtitle ? <p className="max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-300">{subtitle}</p> : null}</div> : null}
            {children}
          </div>
