...
import { ArrowDown, ArrowUp, ArrowUpDown, Disc3, Moon, Sun, ChevronLeft, ChevronRight } from 'lucide-react';
...
import { PageBackdrop, pageCardClass, pageShellClass, secondaryButtonClass, ThemeToggle, tableCellClass } from '@/utils/uiTheme';
...
export default function ArtistAlbumsClient() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    router.replace(`/artists/${canonicalArtistId}/albums${suffix}`);
  }, [canonicalArtistId, currentPage, data?.artist?.id, id, router, order, sort]);
  ...
  const setPage = (page) => {
    ...
    router.push(`/artists/${id}/albums?${next.toString()}`);
  };
              ...
              <ResponsiveResultList
                ...
                renderDesktopRow={(album, index, rowClass) => (
                  <tr key={album.id ?? index} className={rowClass}>
                    ...
                    {isUnit ? <td className={tableCellClass}>{membersText(album.unit_members) || '-'}</td> : null}
                  </tr>
                )}
